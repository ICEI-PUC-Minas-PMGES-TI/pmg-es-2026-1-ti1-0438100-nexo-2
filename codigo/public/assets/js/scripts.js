...
const ROTAS = {
  ...
  detalhesDenuncia:
    "/modulos/detalhes/detalhes.html",
...
async function carregarLayoutUsuario() {
  try {
    ...
    if (morador) {
        const perfil = infoPerfilMoradores.find(info =>
            normalizarCpf(info.usuarioMorador_cpf) === cpf
        );
        ...
        aplicarLayoutMorador(usuarioCompleto);
        return;
    }
    ...
    if (usuarioInstituicao) {
        const perfil =
            infoPerfilInstituicoes.find((info) => {
            return (
                normalizarCpf(
                info.usuarioInstituicao_cpf
                ) === cpf
            );
            });
        ...
        const instituicao =
            ...
            instituicoes,
            usuarioInstituicao.instituicao_id
            );
